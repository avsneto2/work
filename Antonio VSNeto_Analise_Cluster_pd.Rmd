---
title: "Projeto-Pós Graduação, Análise de Cluster."
...
básicos na utilização da linguagem "R", bem como, nos conhecimentos na
análise de Clusters.

Neste documento, segue o passo a passo, onde são demonstrados os
conhecimentos adquiridos na disciplina, "ANALISE DE CLUSTER".

# Objetivo do Projeto

Demonstrar os conhecimentos adquiridos na disciplina "ANALISE DE
CLUSTER", sendo que , através do uso da Linguagem "R", serão feitas
diversas análises na base de dados "student-mat.csv", que possui
informações sobre o desempenho de alunos no ensino secundário.

O objetivo do trabalho será identificar a influência dos pais,
considerando o grau de instrução destes, no desempenho dos alunos.
...
-   3-Instalação de pacotes como "FactoExtra", etc...

# Importando bibliotecas necessárias ao desenvolvimento. -

Para suporte no tratamento da base de dados, foram instaladas várias
bibliotecas, bem como, as bibliotecas "FactoExtra", "png", "cluster",
"grid", "gridExtra", a partir do comando "install.packages".
...
setwd("E:/DADOS/VIEIRA/POS GRADUACAO/INFINET/CURSO/WORK_TRABALHO_5")
getwd()
```

# Executando o algoritmo CLARA

## Executando com 3 cores: 

```{r clara3, echo=TRUE}

# Load the PNG into an RGB image object

lena <- readPNG("Lenna.png")

# Reshape image into a data frame

df <- data.frame(
  red = matrix(lena[,,1], ncol = 1),
  green = matrix(lena[,,2], ncol = 1),
  blue = matrix(lena[,,3], ncol = 1)
)

# Perform CLARA clustering with 3 clusters
num_clusters <- 03 
clustering <- clara(df, num_clusters, samples = 50)
...
R <- matrix(df$red, nrow = dim(lena)[1])
G <- matrix(df$green, nrow = dim(lena)[1])
B <- matrix(df$blue, nrow = dim(lena)[1])

# Reconstitute the segmented image in the same shape as the input image
lena_segmented <- array(dim = dim(lena))
lena_segmented[,,1] <- R
lena_segmented[,,2] <- G
lena_segmented[,,3] <- B
...
# Perform CLARA clustering with 5 clusters
num_clusters <- 05
clustering <- clara(df, num_clusters, samples = 50)
...
# Perform CLARA clustering with 10 clusters
num_clusters <- 10
clustering <- clara(df, num_clusters, samples = 50)
...
selecionado um evento contendo 5 (cinco) países europeus e 5 (cinco)
paises da America do Sul, considerando os anos de 1995 e 2000.
...
Com base na coluna ano, selecionar o movimento dos dois últimos
anos(1995 e 2000).
...
pacote summarytools, para esta análise, considerando o dados referente
ao ano de 1995.
...
Pode-se observar uma leve diferença entre o valor da media (Mean =
10.26) e Mediana (Median =10.57), o que também remete a uma simetria
entre os valores, sem a presença de outliers signficativos.
...
Com base o indicador skewness acima, em 0.27 e sabendo-se que valores de
skew na faixa de -0.5 \< skew \< 0.5, podemos afirmar que trata-se de um
caso fracamente simetrico
...
Com base o indicador skewness acima, em 0.44 e sabendo-se que valores de
skew na faixa de -0.5 \< skew \< 0.5, podemos afirmar que trata-se de um
caso fracamente simetrico
...
Com base o indicador skewness acima, em -0.06 pode-se afirmar que a
distribuição neste caso é normal.
...
Pode-se observar uma diferença entre o valor da media (Mean = 29.89) e
Mediana (Median =30.70), o que também remete a uma simetria entre os
valores, sem a presença significativa de outliers.

Conclusão : Para os dados analisados em 1995, de forma geral, os dados
tendem a uma distribuição normal, sem a presença significativa de
outliers
...
de skew na faixa de 0.5 \< skew \< 1, podemos afirmar que trata-se de um
caso assimetria moderada.
...
Pode-se observar uma leve diferença entre o valor da media (Mean =
10.31) e Mediana (Median =10.58), o que também remete a uma simetria
entre os valores, sem a presença de outliers signficativos.
...
Com base o indicador skewness acima, em 0.35 e sabendo-se que valores de
skew na faixa de -0.5 \< skew \< 0.5, podemos afirmar que trata-se de um
caso fracamente simetrico
...
Pode-se observar uma diferença entre o valor da media (Mean = 30.86) e
Mediana (Median =31.10), o que também remete a uma simetria entre os
valores, sem a presença significativa de outliers.

Conclusão : Para os dados analisados em 2000, de forma geral, os dados
tendem a uma distribuição normal, sem a presença significativa de
outlier.
...
Na analise acima é possivel verificar maior correlação entre as
seguintes variaveis:
...
Até o momento tem um simetria moderada, Também no gráfico, não
observa-se a presença significativa de outliers, confirmando o resultado
das funções summarytools::descr(), executadas acima.
...
Considerando as regressões acima, o modelo que utiliza as variaveis
log.pib.real, idade.mediana e fracao.pop.60_mais é o mais satisfatório.
Dessa forma será feito a imput de dados conforme abaixo, utilizando o
pacote mice.
