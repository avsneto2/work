---
title: "Projeto-Pós Graduação, Estatística para Cientista de Dados."
...
- A análise visa, demonstrar a relação nas ocorrencias de apreensão de drogas e roubos de transeuntes, roubos de residencias e ou roubo de celulares, analisando a concentração de ocorrencias para cada evento.  
...
kable(head(Bevolucao.roubos.f3.tbl, 10), booktabs = TRUE) %>%  kable_styling(font_size = 9)
```
...
Dados calculados para o ano 2020 e Regiao Baixada Fluminense : 

- Média   (Mean)      =  973.46 
- Mediana(Median-Q2)  =  335.00 
- Quartil 25% (Q1)    =  112.00 
- Quartil 75% (Q3)    = 1459.00 
- Desvio Padrao (IQR) = 1347.00
...
Neste caso, a amplitude da binarização foi ajustada para acompanhar a curva de pontos dos dados, onde cheguei a conclusao que a amplitude ideal para a binarização é considerar o valor de 30.
...
-Conclusao :  as variaveis com maior correlação sao: 
                    roubo_celular    x roubo_transeunte 
                    roubo_residencia x roubo_transeunte 
                    roubo_celular    x robuo_residencia 
 

```{r Correlacao de variaveis , echo = TRUE}

kable(cor(Bevolucao.roubos.f1.tbl %>% dplyr::filter (ano =='2022'& Regiao=="Interior") %>% dplyr::select(apreensao_drogas, roubo_celular, roubo_residencia,roubo_transeunte)))

```

-   Scatterplot roubo_celular_sum x roubo_transeunte_sum

```{r analisando scatter entre variaveis mais correlacionadas3 , echo = TRUE}
Bevolucao.roubos.f5.tbl %>% dplyr::select(apreensao_drogas_sum, roubo_transeunte_sum,roubo_celular_sum,roubo_residencia_sum) %>% ggplot(aes(x=roubo_celular_sum, y =roubo_transeunte_sum)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + theme_classic()
```

-   Scatterplot roubo_celular x roubo_residencia 

```{r analisando scatter entre variaveis mais correlacionadas , echo = TRUE}
Bevolucao.roubos.f5.tbl %>% dplyr::select(apreensao_drogas_sum, roubo_transeunte_sum,roubo_celular_sum,roubo_residencia_sum) %>% ggplot(aes(x=roubo_celular_sum, y =roubo_residencia_sum)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + theme_classic()
```

-   Scatterplot roubo_residencia_sum x roubo_transeunte_sum

```{r analisando scatter entre variaveis mais correlacionadas2 , echo = TRUE}
Bevolucao.roubos.f5.tbl %>% dplyr::select(apreensao_drogas_sum, roubo_transeunte_sum,roubo_celular_sum,roubo_residencia_sum) %>% ggplot(aes(x=roubo_residencia_sum, y =roubo_transeunte_sum)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + theme_classic()
```

#### Criando um grafico de barras 

-   Criando um grafico de barras, utilizando 2 variaveis (municipio e roubo de rua)
